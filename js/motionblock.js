    ...
    constructor(params) {
			...
			load(type, prefix, loader, caption, folder, element){
					...
					//THIS.coverComplete = true;
						...
						if (type == "background") THIS.backgroundComplete = false;
				...
				if (type == "background"){
					...
					slide.src = THIS.folder + "/background" + prefix + "/" + THIS.center_num + ".jpg";
					slide.onload = function(){
						if (!THIS.started) {
							THIS.started = true;
							THIS.atFirstLoaded = true;
							THIS.slides[this.num] = this;
							THIS.resize();
				...
				if (type == "object"){
					...
					for (let n = 1; n <= this.images_count; n++){
						...
						cover.num = n - 1;
						cover.src = THIS.folder + "/object" + prefix + "/" + THIS.object_folder + n + ".png";
						cover.onload = function(){
							if (this.num == THIS.center_num - 1) THIS.scan(this);
							loaded_count++;
								...
								let wait = setInterval(function(){
									if (THIS.backgroundComplete || !THIS.started){  //.atFirstLoaded){
										...
										for (let j = 0; j < THIS.covers_pre.length; j++){
											THIS.covers[j] = document.createElement("img");
			...
			full_load(type, prefix, loader, caption, folder){
				...
				for (let n = 1; n <= this.images_count; n++){
					let slide = document.createElement("img");
					slide.num = n - 1;
					slide.src = THIS.folder + "/background" + prefix + "/" + n + ".jpg";
					slide.onload = function(){
							loaded_count ++;
							...
							let wait2 = setInterval(function(){
								...
								if (THIS.coverComplete){
									...
									for (let j = 0; j < THIS.slides_pre.length; j++) try{
										THIS.slides[j] = document.createElement("img");
